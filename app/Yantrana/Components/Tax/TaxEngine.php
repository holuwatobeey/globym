...
class TaxEngine implements TaxEngineBlueprint
{
    ...
    public function addProcess($inputData)
    {
        ...
        if (__isEmpty($country)) {
            return __engineReaction(2, null, __tr('Invalid Country Selected.'));
        }
        ...
        if ($this->taxRepository->store($inputData)) {
            return __engineReaction(1, null, __tr('Tax added successfully.'));
        }

        return __engineReaction(2, null, __tr('oh..no. error.'));
    }
    ...
    public function processUpdate($taxID, $inputData)
    {
        ...
        if (empty($tax)) {
            return __engineReaction(18, null, __tr('Tax does not exist.'));
        }
        ...
        if (__isEmpty($country['iso_code'])) {
            return __engineReaction(14, null, __tr('Invalid Country Code.'));
        }
        ...
        if ($this->taxRepository->update($tax, $updateData)) {
            return __engineReaction(1, null, __tr('Tax updated successfully.'));
        }

        return __engineReaction(14, null, __tr('Nothing update.'));
    }
     ...
     * @return array
     *---------------------------------------------------------------- */
    public function additionOfTaxses($country, $cartTotalPrice, $discountAddedPrice, $discount)
    {
        ...
        if (getStoreSettings('apply_tax_after_before_discount') == 2) {
            $cartTotalPrice = handleCurrencyAmount($cartTotalPrice) - handleCurrencyAmount($discount);
        } elseif (!__isEmpty($discount)) {
            $cartTotalPrice = handleCurrencyAmount($cartTotalPrice) - handleCurrencyAmount($discount);
        }
        ...
        if (__isEmpty($taxCollection)) {
            return [
                'info' => '',
                'formatedTotalTaxAmount' => priceFormat(0, false, true),
                'totalTaxAmount'   => handleCurrencyAmount(0),
                'totalPrice' => __isEmpty($discountAddedPrice)
                                    ? $cartTotalPrice
                                    : handleCurrencyAmount($discountAddedPrice),
            ];
        }
        ...
        foreach ($taxCollection as $key => $tax) {
            $calculatedAmount = handleCurrencyAmount(0);

            //tax type 1 means flat amount
            if ($tax->type == 1) {
                $calculatedAmount = $tax->applicable_tax;
                $applicableTaxAmount[] = handleCurrencyAmount($calculatedAmount);
            }
            ...
            $taxData[$key] = [
                'id'        => $tax->_id,
                'label'     => __transliterate('taxes', $tax->_id, 'label', $tax->label),
                'notes'     => __transliterate('taxes', $tax->_id, 'notes', $tax->notes),
                'type'      => $tax->type,
                'status'    => $tax->status,
                'applicableTax' => $tax->applicable_tax,
                'formatedTax'   => priceFormat($calculatedAmount, false, true),
                'amount'        => handleCurrencyAmount($calculatedAmount),
            ];
        }
        ...
        return [
            'info'       => $taxData,
            'formatedTotalTaxAmount' => priceFormat($totalTaxAmount, false, true),
            'totalTaxAmount'   => handleCurrencyAmount($totalTaxAmount),
            'totalPrice' => $totalTaxAmount + $discountAddedPrice,
        ];
    }
    ...
    public function getTaxInformation($taxsesData, $currency)
    {
        ...
        foreach ($taxCollection as $key => $tax) {
            ...
            $taxArray[] = [
                'label' =>  __transliterate('taxes', $tax->_id, 'label', $tax->label),
                'baseTaxLabel' =>  $tax->label,
                'notes' =>  __transliterate('taxes', $tax->_id, 'notes', $tax->notes),
                'baseTaxNotes' =>  $tax->notes,
                'taxAmount' => $amount,
                'formatedTaxAmount' => priceFormat($amount, $currency),
                'baseFormatedTaxAmount' => basePriceFormat($amount, $currency)
            ];
        }
