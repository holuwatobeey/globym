...
class AddressEngine implements AddressEngineBlueprint
{
    ...
    public function prepareList()
    {
        ...
        if (empty($getAddress)) {
            return __engineReaction(2, [
                'message' => __tr('There are no address.'),
            ]);
        }
    ...
    public function getUserAddress($addressID)
    {
        ...
        if (!empty($address)) {
            ...
            $getAddress = [
                ...
                'pin_code' => $address->pin_code,
                'countryCode' => $address->country,
                'countries_id' => $address->countries__id
            ];
        }
    ...
    public function checkIsValidAddress($data, $inputAddress)
    {
        ...
        if (!isLoggedIn()) {
            if (__isEmpty($shippingAddress) or ($shippingAddress->users_id != $inputAddress['current_user_id'])) {
                return 3;
            }
        } else {
        ...
        // if check the tax apply after discount
        if (getStoreSettings('calculate_tax_as_per_shipping_billing') == 2) {
            $billing = $this->addressRepository->fetchAddressByID($addressID1);
    ...
    /**
     * get address by shipping address id & billing address id.
     *
     * @param int $addressId
        ...
        if (!__isEmpty($address)) {
           ...
           // fetch primary address of user
            ...
            if ($address->id !== (int) $addressId1) {
                ...
                $billingCountry = $this->supportRepository->fetchCountry($address1->countries__id);
     ...
     * @return array
     *---------------------------------------------------------------- */
    public function storeAddresses($inputData)
    {
        $shippingAddressData = [];
        $billingAddressData = [];
        $addressIds = [];
        $shippingCountry = $this->supportRepository->fetchCountry($inputData['shipping_country']);

        $shippingAddressData = [
            'type'              => $inputData['shipping_address_type'],
            'address_line_1'    => $inputData['shipping_address_line_1'],
            'address_line_2'    => $inputData['shipping_address_line_2'],
            'city'              => $inputData['shipping_city'],
            'state'             => $inputData['shipping_state'],
            'country'           => $shippingCountry->iso_code,
            'pin_code'          => $inputData['shipping_pin_code'],
            'status'            => 1,
            'countries__id'     => $inputData['shipping_country'],
            'users_id'          => $inputData['guest_user_id']
        ];

        $shippingAddress = $this->addressRepository->storeAddress($shippingAddressData);
        $addressIds['addressId1'] = $shippingAddress->id;
        $addressIds['addressId2'] = $shippingAddress->id;
        $addressIds['shippingCountry'] = $shippingCountry->iso_code;
        $addressIds['billingCountry'] = $shippingCountry->iso_code;

        if ($inputData['use_as_billing'] != 1) {

            $billingCountry = $this->supportRepository
                                    ->fetchCountry($inputData['billing_country']);

            $billingAddressData = [
                'type'              => $inputData['billing_address_type'],
                'address_line_1'    => $inputData['billing_address_line_1'],
                'address_line_2'    => $inputData['billing_address_line_2'],
                'city'              => $inputData['billing_city'],
                'state'             => $inputData['billing_state'],
                'country'           => $billingCountry->iso_code,
                'pin_code'          => $inputData['billing_pin_code'],
                'status'            => 1,
                'countries__id'     => $inputData['billing_country'],
                'users_id'          => $inputData['guest_user_id']
            ];

            $billingAddress = $this->addressRepository->storeAddress($billingAddressData);

            $addressIds['addressId2'] = $billingAddress->id;
            $addressIds['billingCountry'] = $billingCountry->iso_code;
        }
