...
class MediaEngine implements MediaEngineBlueprint
{
    ...
    public function processUploadedImage($imageFile)
    {
        ...
        $fileBaseName = slugIt(basename($fileName, '.'.$fileExtension));
        ...
        if ($imageFile->move($path, $fileName)) {
            $thumbnailURL = getUserTempImageURL($fileName, getUserId());
    ...
    public function prepareUploadedImagesFiles()
    {
        $userID = Auth::id();
        $files = [];
        $tempFiles = glob(getLoggedInUserTempMediaPath().'*.{jpg,jpeg,png,gif,ico}', GLOB_BRACE);
     ...
     * @return bool
     *---------------------------------------------------------------- */
    public function storeSliderMedia($fileName, $sliderID, 
     $removeExistingFileName = null, $generateThumbnail = false, $saveOriginalFileForZoom = false)
    {
       
        $sliderMediaPath = getSliderMediaPath($sliderID);
        $sourcePath = getLoggedInUserTempMediaPath().$fileName;
        $destinationPath = $sliderMediaPath.'/'.$fileName;

        //Check if product media directive exist
        if (!File::isDirectory($sliderMediaPath)) {
            File::makeDirectory($sliderMediaPath, $mode = 0777, true, true);
        }
        ...
        if (File::move($sourcePath, $destinationPath)) {
            ...
            if (!__isEmpty($removeExistingFileName)
                and File::exists($destinationPath) 
                and File::exists($sliderMediaPath.'/'.$removeExistingFileName)) {

                if (!File::delete($sliderMediaPath.'/'.$removeExistingFileName)) {
                    return false;
                }
            }
     ...
     * @return bool
     *---------------------------------------------------------------- */
    public function storeHomePageContentMedia($fileName, $folderName, $removeExistingFileName = null)
    {
       
        $homePageMediaPath = getHomePageContentMediaPath($folderName);
        $sourcePath = getLoggedInUserTempMediaPath().$fileName;
        $destinationPath = $homePageMediaPath.'/'.$fileName;

        //Check if product media directive exist
        if (!File::isDirectory($homePageMediaPath)) {
            File::makeDirectory($homePageMediaPath, $mode = 0777, true, true);
        }
        ...
        if (File::exists($sourcePath)) {
	        if (File::move($sourcePath, $destinationPath)) {
	            if (!empty($removeExistingFileName)
	                and File::exists($homePageMediaPath.'/'.$removeExistingFileName)) {
	                if (!File::delete($homePageMediaPath.'/'.$removeExistingFileName)) {
	                    return false;
	                }
	            }
    ...
    public function deleteSliderMedia($sliderID, $fileName = null)
    {
        $sliderMediaPath = getSliderMediaPath($sliderID);

        $path = $sliderMediaPath;

        if (!__isEmpty($fileName)) {
            $path = $sliderMediaPath.'/'.$fileName;
        } 
     ...
     $removeExistingFileName = null, $generateThumbnail = false, $saveOriginalFileForZoom = false)
    {
        ...
        $bigFileDestinationPath = $productMediaPath.'/'.'big_'.$fileName;
        ...
        if (File::move($sourcePath, $destinationPath)) {
            ...
            if ($saveOriginalFileForZoom) {
                File::copy($destinationPath, $bigFileDestinationPath);
            } 
     ...
     * @return bool
     *---------------------------------------------------------------- */
    public function processFaviconMedia($logoImageFile)
    {   
        ...
        $logoMediaPath = getFaviconMediaPath();
        ...
        $faviconName = config('__tech.faviconName');

        // Set name for image
        $destinationPath = $logoMediaPath.'/'.$faviconName;
     ...
     * Process store logo media.
     *
     * @param string $invoiceImageFile
     *
     * @return bool
     *---------------------------------------------------------------- */
    public function processStoreSettingInvoiceMedia($invoiceImageFile)
    {
        $sourcePath = getLoggedInUserTempMediaPath().$invoiceImageFile;
        ...
        // Get path Info about selected logo
        $invoiceImageInfo = pathinfo($sourcePath);

        // Get extension of selected logo
        $fileExtension = $invoiceImageInfo['extension'];
        ...
        // Get source and media path
        $invoiceMediaPath = getInvoiceMediaPath();

        //Check if logo media directory exist
        if (!File::isDirectory($invoiceMediaPath)) {
            File::makeDirectory($invoiceMediaPath, $mode = 0777, true, true);
        }

        // Default Logo Image File Name
        $invoiceName = config('__tech.invoiceName');

        // Set name for image
        $destinationPath = $invoiceMediaPath.'/'.$invoiceName;
     ...
     * @return array
     *---------------------------------------------------------------- */
    public function processUploadImageByExtension($imageFile, $extensionsAllowed = null)
    {
        // Check if file empty or is valid
        if (empty($imageFile) and !$imageFile->isValid()) {
            return __engineReaction(2, null, 'Invalid request');
        }

        $fileExtension = $imageFile->getClientOriginalExtension();

        // if allowed file is required like some extensions only
        if (!__isEmpty($extensionsAllowed)) {
            $definedExtensions = configItem('media.extensions.'.$extensionsAllowed);
            ...
            if (!in_array(strtolower($fileExtension), $definedExtensions)) {
	           	return __engineReaction(2, null, __tr('File Has An Invalid Extension, It Should Be __extensions__', [
