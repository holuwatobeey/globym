...
class ManageProductRepository extends BaseRepository implements ManageProductRepositoryBlueprint
{
    /**
    ...
    protected $productRating;
     ...
     /**
     * @var productQuestion - productQuestion Repository
     */
    ...
    public function store($input)
    {
        ...
        $related_products = array_get($input, 'related_products', null);
        ...
        // Check if old price entered
        if (isset($input['launching_date']) && $input['out_of_stock'] == 3) {
            $launchingDate = convertDateTimeZone(Carbon::parse($input['launching_date'])
            					->toDateTimeString(), $input['getClientTimeZone']);
          
            $jsonData = [
                'launching_date' => Carbon::parse($launchingDate)->toDateTimeString()
            ];

            $newProduct->__data = $jsonData;
        }
    ...
    public function fetchAllProduct()
    {
     ...
     /**
      * Fetch product rating datatable source
      *
      * @return array
      *---------------------------------------------------------------- */
 
    public function fetchProductRatingDataTableSource()
    {   
        $dataTableConfig = [
            'searchable' => [
        ...
        return $product->join('product_ratings', 'products.id', 
                            ...
                            ->select(
                                __nestedKeyValues([
                                    ...
                                    'product_ratings' => [
                                        '_id',
                                        DB::raw('
                                        ROUND(AVG(product_ratings.rating), 1) as productRating'),
                                        'products_id',
                                        'created_at'
                                    ]
                                ])
                            ...
                            ->groupBy('products.id')
                            ->latest('product_ratings.created_at')
    ...
    public function fetchLatestProductRating()
    {   
        $product = $this->product;
        ...
        return $product->join('product_ratings', 'products.id', 
                            ...
                            ->groupBy('products.id')
                            ->latest('product_ratings.created_at')
                            ->take(configItem('product_rating_on_db_count'))
                            ->get();
    }
    ...
    /**
     * fetch multiple category by category ids.
     *
     * @param int $categoryIds
     *
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchMultipleCategories($categoryIds)
    {
    ...
    public function fetchProductCategory($productID)
    {
        return $this->productCategory
                    ...
                    ->where('product_categories.products_id', $productID)
     ...
     * @return bool
     *---------------------------------------------------------------- */
    public function updateProduct($product, $updateData)
    {
    ...
    public function update($product, $input)
    {
        ...
        $jsonData = [];
        // Check if launching on date entered
        if (isset($input['launching_date']) && $input['outOfStock'] == 3) {
 
            $launchingDate = convertDateTimeZone(Carbon::parse($input['launching_date'])->toDateTimeString(), $input['getClientTimeZone']);
            $jsonData = [
                'launching_date' => Carbon::parse($launchingDate)->toDateTimeString()
            ];

            $updateData['__data'] = $jsonData;
        } 
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchMaxListOrder($productID)
    {
        return  $this->productImage->where('products_id', $productID)
                            ->max('list_order');
    }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchProductImages($productID, $optionValuesIds = [])
    {   
        if (!__isEmpty($optionValuesIds)) {

            $query = $this->productImage->where(function($qu) use($productID, $optionValuesIds) {
                $qu->whereIn('product_option_values_id', $optionValuesIds)
                  ->where('products_id', $productID);
            })->orWhere(function($q) use($productID) {
                $q->whereNull('product_option_values_id')
                   ->where('products_id', $productID);
            });

        } else {

            $query = $this->productImage->whereNull('product_option_values_id')
                   ->where('products_id', $productID);
        }
        ...
        return $query->select(
                ...
                'file_name', 
                'list_order',
                'products_id', 'product_option_values_id'
            )->get()->toArray();
    }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchProductImage($productID, $id)
    {  
        return $this->productImage
                    ->where('products_id', $productID)
                    ->where('id', $id)
     ...
     * @return bool
     *---------------------------------------------------------------- */
    public function updateImageListOrder($productImageData)
    {
        // Check if pages list order updated
        if ($this->productImage->batchUpdate($productImageData, 'id')) {
    ...
    {
        $optionValues = $productImages = [];

        $isUpdate = false;

        foreach ($values as $value) 
        {
            $optionValueModelInstance = new $this->productOptionValue;

            $optionValues = [
                'name' => $value['name'],
                'image_name'   => isset($value['image']) ? $value['image'] : null,
            ...
            if ($optionValueModelInstance->assignInputsAndSave($value, $optionValues)) {

                $opValueId = $optionValueModelInstance->id;

                if (!__isEmpty($value['slider_images'])) {

                    foreach ($value['slider_images'] as $pImageId) {

                        $productImages[] = [
                            'id' => $pImageId,
                            'product_option_values_id' => $opValueId
                        ];
                    }
                }
        ...
        if (!__isEmpty($productImages) and ($this->productImage->batchUpdate($productImages, 'id'))) {
            $isUpdate = true;
        }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function checkValueImagesExist($productId, $optionID)
    {
        return $this->productImage
                    ->leftJoin('product_option_values', 'product_images.product_option_values_id', '=', 'product_option_values.id')
                    ->where('product_images.products_id', $productId)
                    ->whereNotIn('product_option_labels_id', [$optionID])
                    ->get();
    }

    /**
     * Check prodct images exist
     *
     * @param number $productId
     *
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function checkProductImagesExist($productId)
    {
        return $this->productImage
                    ->where('products_id', $productId)
                    ->whereNotNull('product_option_values_id')
                    ->exists();
    }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchProductOptionsValuesByIds($optionIDs, $optionValueIDs)
    {
        return $this->productOptionValue
                    ->with('productOptionLabel')
                    ->whereIn('product_option_labels_id', $optionIDs)
    ...
    {
        if ($this->productOptionValue->batchUpdate($optionValues, 'id')) {
            return true;
        }

        // if ($this->productOptionValue('product_option_values', $optionValues, 'id')) {
        //     return true;
        // }
    ...
    public function updateNewOptionValues($productID, $optionID, $newInputValues)
    {
        ...
        {   
            ...
            $newOptionValues = [
                ...
                'product_option_labels_id' => $optionID,
                'image_name' => $newInputValue['image_name'],
                'addon_price' => isset($newInputValue['addon_price'])
                                               ? $newInputValue['addon_price']
                                               : 0
            ];

            if ($optionValueModelInstance->assignInputsAndSave($newInputValue, $newOptionValues)) {

                $opValueId = $optionValueModelInstance->id;

                if (!__isEmpty($newInputValue['slider_images'])) {

                    foreach ($newInputValue['slider_images'] as $pImageId) {
        ...
        if (!__isEmpty($productImages)
         and ($this->productImage->batchUpdate($productImages, 'id'))){
            $isUpdate = true;
            activityLog('ID of '.$productID.' product option values updated.');
        }
    ...
    {
        return $this->product->where('out_of_stock', 1)
                    ->count();
    }
    ...
    public function fetchSpecificationDataTableSource($productID)
    {
        return $this->productSpecification
                    ->leftjoin('specification_values', 'product_specifications.specification_values__id', '=', 'specification_values._id')
                    ->where('product_specifications.products_id', $productID)
                    ->select(
                        __nestedKeyValues([
                            'product_specifications' => [
                                '_id',
                                'specifications__id',
                                'specification_values__id'
                            ],
                            'specification_values' => [
                                '_id as specificationValueId',
                                'specification_value as value'
                            ]
                        ])
                    )
                    ->get();
    ...
    /**
     * Fetch product old specification datatable source.
     *
     * @param number productID
     *
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchExistingSpecificationData($productID)
    {
        return $this->productSpecification
                    ->where('product_specifications.products_id', $productID)
                    ->get();
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchSpecificationValues()
    {
        return SpecificationValue::get();
    }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchProductSpecificationByProductId($productID)
    {
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchProductSpecificationBySpecId($productIds)
    {
        return $this->productSpecification
                    ->leftjoin('specifications', 'product_specifications.specifications__id', '=', 'specifications._id')
                    ->whereIn('product_specifications.products_id', $productIds)
                    ->select(
                        __nestedKeyValues([
                            'product_specifications' => [
                                '_id', 
    ...
    /**
     * Store Category or Product Preset.
     *
     * @param array  $inputData
     *
     * @return bool
     *---------------------------------------------------------------- */
    public function storeCategoryOrProductPreset($productID, $inputData)
    { 
        ...
        if (!__isEmpty($inputData)) {
            foreach ($inputData as $key => $input) {
                ...
                if (__isEmpty($input['valueExists'])) {
                    $specificationValue = new SpecificationValue;
                    $keyValues = [
                        'specification_value' => $input['value'],
                        'specifications__id' => $input['specifications__id'],
                        'status' => 1
                    ];

                    if ($specificationValue->assignInputsAndSave($input, $keyValues)) {
                        $specificationValueId = $specificationValue->_id;
                        $specificationValueIds[] = $specificationValueId;
                        
                        $productSpecificationInsert[] = [
                            'specification_values__id' => $specificationValueId,
                            'specifications__id' => $input['specifications__id'],
                            'products_id' => $productID
                        ];
                    }
                ...
                if (!__isEmpty($input['valueExists'])) {
                    $productSpecificationInsert[] = [
                        'specification_values__id' => $input['specValueId'],
                        'specifications__id' => $input['specifications__id'],
                        'products_id' => $productID
                    ];
                }
            ...
            if ($this->productSpecification->prepareAndInsert($productSpecificationInsert)) {
                activityLog('ID of '.$productID.' product custom specification added.');
                return true;
            }
        }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function storeSpecifications($storeData, $productID)
    {
        ...
        if ($specification->assignInputsAndSave($storeData, $keyValues)) {
            $productSpecifications = [];
            foreach ($storeData['values'] as $key => $value) {
                $productSpecifications[] = [
                    'value' => $value,
                    'products_id' => $productID,
            ...
            foreach ($productSpecifications as $key => $input) {
    ...
    public function fetchProductRatingDatabaleSource($productId)
    {
        $dataTableConfig    = [
            'fieldAlias' => [],
            'searchable' => [
        ...
        return $this->productRating->join('users', 'users.id', '=', 'product_ratings.users_id')
                    ->where([
                        'products_id' => $productId,
                        'users.status' => 1
                    ])
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchFaqByID($faqID)
    {
    ...
    public function fetchNotifyUser($notifyUserId)
    {
        return $this->productNotification->where('_id', $notifyUserId)->first(); 
    }
    ...
    public function fetchProductNotifyByProductId($productId)
    {
    ...
    public function deleteMultipleAwatingUser($productId, $deleteAwatingUserIds)
    {
        return $this->productNotification->where([
                        'products_id' => $productId
                    ])
                    ->whereIn('_id', $deleteAwatingUserIds)
                    ->delete();
    }

    /**
      * Delete Awating Users
      *
      * @param object $task
      *
      * @return  mixed
    ...
    public function deleteAwatingUser($awatingUser)
    {
        if ($awatingUser->delete()) {
            return true;
        }
    ...
    public function fetchProductFaqDatabaleSource($productId)
    {
        $dataTableConfig    = [
            'fieldAlias' => [],
            'searchable' => [
        ...
        return $this->productQuestion->join('users', 'users.id', '=', 'product_questions.users_id')
                    ->where([
                        'products_id' => $productId,
                        'users.status' => 1
                    ])
     ...
     * @return bool
     *---------------------------------------------------------------- */
    public function storeFaqValues($storeData)
    {   
        $keyValues = [
            'products_id',
            'users_id',
     ...
     * @return bool
     *---------------------------------------------------------------- */
    public function updateFaq($faqData, $input)
    {
        if ($this->productQuestion->where('_id', $faqData->_id)
                                        ->update([
                                        'question' => $input['question'],
                                        'answer' => $input['answer'],
                                    ])) {

            activityLog('ID of '.$faqData->products_id.' product faq updated.');
    ...
    public function deleteFaq($faqData)
    {
        // Check if faq deleted
        if ($faqData->delete()) {

            activityLog("Id of $faqData->_id item faq deleted");
    ...
    public function fetchRating($ratingId)
    {
        return $this->productRating->where('_id', $ratingId)->first();
    }
    ...
    {
        // Check if rating deleted
        if ($rating->delete()) {

            activityLog("Id of $rating->_id item rating deleted");
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchCategoriesProducts($categoryIDs, $inactiveBrandIds = [])
    {
        return $this->product
                    ->sortOrder()
    ...
    /**
     * Fetch Product Count by Preset Id
     *
     * @param int $presetId
     *
     * @return Eloquent collection object
     *-----------------------------------------------------------------------*/
    public function fetchProductCountByPreset($presetId)
    {
        return $this->product->where('specification_presets__id', $presetId)->count();
    }

    /**
     * Delete Product Specification by product id
     *
     * @param int $productID
     *
     * @return Eloquent collection object
     *-----------------------------------------------------------------------*/
    public function deleteProductSpecification($productID)
    {
        return $this->productSpecification->where('products_id', $productID)->delete();
    }
     ...
     * @return Eloquent collection object
     *-----------------------------------------------------------------------*/
    public function batchUpdate($data, $columnName)
    {
        if ($this->productImage->batchUpdate($data, $columnName)) {
            return true;
        }
