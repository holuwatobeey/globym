...
class ManageCategoryRepository extends BaseRepository
{
    ...
    /**
     * fetch all active categories with its children
     *
     * @return Eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchAllActiveWithChildren()
    {
        return $this->viaCache('cache.categories.with.child.all', function () {
            return $this->category
                        ->with([
     ...
     * @return Eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchActiveCategoriesWithChildren($categoryId)
    {
        return $this->category
                    ->where('id', $categoryId)
                    ->with([
                        'children' => function($query) {
                            $query->select('id', 'name', 'parent_id', 'status')->whereStatus(1);
                        }
                    ])
                    ->whereStatus(1)
                    ->select('id', 'name', 'parent_id', 'status')
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchCategorySearchSuggestList($searchQuery, $activeCategoryIds = [])
    {
        $query = $this->category
        				->with([
        ...
        if (!__isEmpty($activeCategoryIds)) {
            $query->whereIn('id', $activeCategoryIds);
        }
    ...
    public function fetchCategories($categoryID, $searchText = null)
    {
        ...
        return $query->with([
                        'childCategoriesRecursive' => function($query) {
                            $query->selectField();
                        }
                    ])
                    ->selectField()
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchAllProductsCategories()
    {
        // fetch categories products count
        return  $this->productCategory
                     ->select('id', 'categories_id', 'products_id')
                     ->get();
    }
    ...
    /**
     * Get direct lebel parent categories.
     *
     * @param int $productIds
     *
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchByProductIds($productIds)
    {
        return $this->productCategory
                    ->leftJoin('categories', 'product_categories.categories_id', '=', 'categories.id')
                    ->whereIn('product_categories.products_id', $productIds)
                    ->select(
                        __nestedKeyValues([
                            'categories' => [
                                'id',
                            ...
                            'product_categories' => [
                                ...
                                'categories_id',
                                'products_id',
                                DB::raw('COUNT(products_id) as productCount')
                            ]
                        ])
                    )
                    ->groupBy('product_categories.categories_id')
                    ->get();
    }
}