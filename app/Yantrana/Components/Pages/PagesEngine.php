...
class PagesEngine implements PagesEngineBlueprint
{
    ...
    {	
		...
		if ($pageID !== 1) {
			$data['breadCrumb'] = Breadcrumb::generate('pages', $pageID);
		}
        ...
        if (!__isEmpty($page)) {
           ...
           // Check if this page is Deactive
	        ...
	        $data['pageDetails'] = [
	            'title'       => __transliterate('menu_pages', $page['id'], 'title', $page['title']),
	            'id'          => $page->id,
	            'description' => __transliterate('menu_pages', $page['id'], 'description', $page['description']),
