...
class UpdateItemInCartRequest extends BaseRequest
{
    ...
    public function messages()
    {
        ...
        if (is_array($values) and !empty($values)) {
            foreach ($values as $key => $value) {
                $messages['items.'.$key.'.qty'.'.max'] = __tr('The quantity may not be greater than 9999');
                $messages['items.'.$key.'.qty'.'.min'] = __tr('The quantity must be at least 1');
                $messages['items.'.$key.'.qty'.'.required'] = __tr('The quantity field is required.');
                $messages['items.'.$key.'.qty'.'.integer'] = __tr('The quantity must be an integer.');
            }
        }
