...
class OrderRepository extends BaseRepository implements OrderRepositoryBlueprint
{
    /**
    ...
    public function orderProcess($inputs)
    {
        ...
        $dataToStore = [
            ...
            'payment_method',
            'payment_status',
            'users_id' => (isLoggedIn()) ? Auth::id() : $inputs['users_id'],
            'order_uid',
            'name',
            'coupons__id',
            'currency_code',
            ...
            'discount_amount', 
            'total_amount',
            '__data' => (!__isEmpty($inputs['additionalDiscountData'])) ? $inputs['additionalDiscountData'] : null
        ]; 

        // If PayPal Live or Sandbox
        if (($paymentMethod == 1) or ($paymentMethod == 7)) {
            if (configItem('env_settings.paypal_test_mode') == true) {
                $dataToStore['business_email'] = getStoreSettings('paypal_sandbox_email');
            } else {
                $dataToStore['business_email'] = getStoreSettings('paypal_email');
            }
        } else {
            $dataToStore['business_email'] = getStoreSettings('business_email');
        }
        ...
        if ($order->assignInputsAndSave($inputs, $dataToStore)) {
            ...
            if (!__isEmpty($inputs['taxses']) and is_array($inputs['taxses'])) {
                ...
                foreach ($inputs['taxses'] as $tax) {
                    $taxesData[] = [
                        'tax__id' => $tax['id'],
                        'orders__id' => $orderID,
                        'amount' => handleCurrencyAmount($tax['amount']),
                    ];
                }
            ...
            if (!__isEmpty($inputs['cartItems'])) {
                foreach ($inputs['cartItems'] as $cartItem) {
                    ...
                    $orderProduct->price = handleCurrencyAmount($cartItem['price']);
                        ...
                        if (__ifIsset($cartItem['options']) and is_array($cartItem['options'])) {
                            ...
                            foreach ($cartItem['options'] as $option) {
                                $optionsData[] = [
                                    ...
                                    'addon_price' => handleCurrencyAmount($option['addonPrice']),
                                ];
                            }
	...
	/**
     * Fetch Purchase item of user
     *
     * @return array
     *---------------------------------------------------------------- */

    public function checkIsPurchasedProductByUser()
    {
        return $this->orderModel
	 ...
	 /**
     * Fetch Purchase order of user
     *
     * @return array
     *---------------------------------------------------------------- */

    public function checkOrderedProducts($orderIds)
    {
        return $this->orderProduct
                    ->whereIn('orders__id', $orderIds)
                    ->pluck('products_id')
                    ->all();
    }

     /**
     * Fetch coupon usage count
     *
     * @return array
     *---------------------------------------------------------------- */
    public function fetchCouponUsage($userId, $couponId)
    {
        return $this->orderModel
                    ->where([
                        'users_id'      => $userId,
                        'coupons__id'   => $couponId
                    ])
                    ->count();
    }
}   
