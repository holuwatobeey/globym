...
class PaypalEngine implements PaypalEngineBlueprint
{
    ...
    public function processIpnRequest()
    {
        ...
        if ($validatedIpnRequest === true) {
            ...
            return $this->orderPaymentsEngine->notifyPaymentConfirmation($this->orderDetails['_id'], $this->ipnInformation);
        }
    ...
    protected function validateIpnInformation()
    {
        ...
        if (($this->validateOrderItem('business_email', $this->ipnInformation['receiver_email']) == false)) {
            ...
            if (configItem('env_settings.paypal_test_mode') == true) {
                $otherValidBusinessEmails[] = getStoreSettings('paypal_sandbox_email');
            }

            // check that receiver_email match with site business email
            if ((in_array($this->ipnInformation['receiver_email'], $otherValidBusinessEmails)) or in_array($this->ipnInformation['business'], $otherValidBusinessEmails)) {
                // email is ok
            } else {
