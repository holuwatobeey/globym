...
class ReportRepository extends BaseRepository implements ReportRepositoryBlueprint
{
    /**
     ...
     *---------------------------------------------------------------- */
    public function fetchDataTableSource($startDate, $endDate, $status, $order, $currency)
    {
        $dataTableConfig = [
            'fieldAlias' => [
                '_id' => '_id',
        ...
        //set filter using currency
        if ($currency == 'null') { 
            $query->where('orders.currency_code', '=', getStoreSettings('currency_value'));
        } else {
            $query->where('orders.currency_code', '=', $currency);
        }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchAllOrder($startDate, $endDate, $status, $order)
    {
        // If order status any and date is created_at
        if ($status != 9 and $order == 1) { // all
    ...
    public function fetchProductReportDataTable($startDate, $endDate, $order)
    {   
        $dataTableConfig = [
            'searchable' => [
                'name'            
        ...
        if ($order == 1) { // all
            ...
            $query = ProductModel::whereBetween(DB::raw('DATE(products.created_at)'), [$startDate, $endDate]);

        // If order status any and date is updated_at
        } elseif ($order == 2) {
            $query = ProductModel::whereBetween(DB::raw('DATE(products.updated_at)'), [$startDate, $endDate]);
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchDataProductReport($startDate, $endDate, $order)
    {
         // If order status any and date is created_at
        if ($order == 1) { // all
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchDataOrderReport($startDate, $endDate, $status, $order, $currency)
    {
        // If order status any and date is created_at
        if ($status != 9 and $order == 1) { // all
    ...
    /**
     * Fetch all order payment data
     *
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchAllOrderPayment($startDate, $endDate, $status, $order)
    {   
             // If order status any and date is created_at
        if ($status != 9 and $order == 1) { 
            // all
            $query = $this->orderPaymentsModel
                          ->leftjoin('orders', 'order_payments.orders__id', '=', 'orders._id')
                          ->whereBetween(DB::raw('DATE(orders.created_at)'), [$startDate, $endDate])
        ...
        return $query->select(
                    ...
                    ->groupBy('order_payments.currency_code')
                    ->get();
    }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchTotalAmountByCurrency($startDate, $endDate, $status,  $order, $currency)
    {        
        // If order status any and date is created_at
        if ($status != 9 and $order == 1) { 
            // all
        ...
        //set filter using currency
        if ($currency == 'null') { 
            $query->where('order_payments.currency_code', '=', getStoreSettings('currency_value'));
        }  else {
            $query->where('order_payments.currency_code', '=', $currency);
        }
     ...
     * @return eloquent collection object
     *---------------------------------------------------------------- */
    public function fetchOrderCollection($startDate, $endDate, $status, $order, $currency)
    {
        // If order status any and date is created_at
        if ($status != 9 and $order == 1) { // all
