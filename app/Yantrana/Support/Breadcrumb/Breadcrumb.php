...
class Breadcrumb
{
    ...
    public function generate($dataType = null, $id = null, $options = [])
    {
        ...
        switch (true) {
            ...
            case $dataType == 'productAllReviews':
                $breadCrumb = $this->productQuestionDetails($dataType, $id, $options, 'productReviews');
            break;

            case $dataType == 'productAllQuestions':
                $breadCrumb = $this->productQuestionDetails($dataType, $id, $options, 'productQuestions');
            break;
            ...
            case $dataType == 'productsCompare':
                $breadCrumb = $this->productCompare($dataType);
            break;
    ...
    /**
     * This method use for display product compare breadCrumb.
     *
     * @param string $productType
     *
     * @return array
     *---------------------------------------------------------------- */
    private function productCompare($productCompareType)
    {   
        if (!empty($productCompareType)) {
    ...
    public function brandRelatedProduct($dataType, $brandID)
    {
        if ($dataType == 'brandProduct') {
            ...
            $collectionBreadcrumb = [
                'parents' => [$brand],
                'title' => __transliterate('brand', $brandID, 'name', $brandName),
            ];
     ...
     * @return array
     *---------------------------------------------------------------- */
    private function productQuestionDetails($featuredType, $productID, $options, $type)
    {
        $product = $this->getProductDetails($productID);
        $pageType = $options['pageType'];
        ...
        if (!empty($product)) {
            ...
            case $pageType == 'featured':
                ...
                $productData[] = $this->getProductBreadcrumbDetails($product['id'], $pageType);

                $pageTypeData = array_merge(
                                    $this->getProductBreadcrumb(),
                                    $pageTypeData, $productData
                                );
            ...
            case $pageType == 'products':
                ...
                $pageTypeData = (array_merge($this->getProductBreadcrumb(), $productData));
            ...
            case $pageType == 'categories':
                ...
                if (!__isEmpty($options['categoryID'])) {
                    ...
                    $category = $this->getProductQuestionWithCategories($product['id']);
     ...
     * @param number $productID
     *---------------------------------------------------------------- */
    private function getProductBreadcrumbDetails($productID, $type)
    {
        $productData = [];
        $pageType = isset($type) ? '?page_type='.$type : '';
        ...
        if (!empty($product->categories)) {
            foreach ($product->categories as $categories) {
                $categoriesData = [
                    'id' => $categories->id,
                    'name' => __transliterate('category', $categories->id, 'name', $categories->name),
                    'slugName' => slugIt($categories->name),
                ];
            }
        }

        if (!empty($product) && $type == 'products' || $type == 'featured') {
            $url = productsDetailsRoute(
                                $product->id,
                                slugIt($product->name)
                            ).$pageType;
        ...
        if (!empty($product) && $type == 'categories') {
            $url = productsDetailsRoute(
                                $product->id,
                                slugIt($product->name),
                                isset($categories->id) ? $categories->id : null
                            ).$pageType;
    ...
    {
        $refinedCategories = findParents(getAllCategories(), (int)$categoryID);

        $getCategory = CategoryModel::whereIn('id', $refinedCategories)->get();
        
        $categoryData = [];

        if (!__isEmpty($getCategory)) {
     ...
     * @param number $categoryID
     *---------------------------------------------------------------- */
    private function getProductQuestionWithCategories($productID = null)
    {
        $product = ProductModel::with('categories')
                                ->where([
                                    'id' => $productID,
    ...
    {
        foreach ($categories as $catKey => $row) {
            if ($row['id'] == $categoryID) {
                $items[] = [
                    'id' => $row['id'],
                    'name' => __transliterate('category', $row['id'], 'name', $row['name']),
    ...
    private function users($userType)
    {
        ...
        if (!empty($userType)) {
            switch (true) {
                ...
                case $userType == 'my-wishlist':
                    $title = __('My Wish List');
                break;

				case $userType == 'my-ratings':
                    $title = __('My Ratings');
                break;
